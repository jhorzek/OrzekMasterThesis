#' Simulation study 3
#' 10 percent of cross-lagged parameters non-zero
#'
#'
...
#'
#'
simulation3_10 <- function(sampleSize, seed, wd, total_repetitions, crossEffect, autoEffect){
  setwd(wd)
  ##### Settings #####
  ...
  Avalues <- diag(autoEffect,nrow=5,ncol = 5)
  ...
  Svalues <- diag(1-autoEffect^2,nrow=5,ncol = 5)
  Svalues[1,1] <- 1-autoEffect^2 -crossEffect^2
  Svalues[3,3] <- 1-autoEffect^2 -crossEffect^2
  ...
  while(iteration <= total_repetitions){
    ...
    # simulate ARCL-SEM:
    simObj <- simARCL_scaled_10(sampleSize = sampleSize, autoEffect = autoEffect, crossEffect = crossEffect)
...
############################################BIS HIER GEKOMMEN ###########################
  ...
  save(overall_evaluation, parameterValues, RMSE, total_repetitions, Avalues, penAelement, Svalues, error, seed, crossEffect, autoEffect, file = paste("Simulation3_N", sampleSize, "_10percent_1_1000.RData", sep = ""))
